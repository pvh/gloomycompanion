...
function dispatch(event_name, payload){
    ...
    var details = { detail: (payload || {}) };

    document.body.dispatchEvent(new CustomEvent(event_name, details));
}

function listen(event_name, callback, context){
    if (!event_name) return;

    callback = callback || (() => {});
    document.body.addEventListener(event_name, (e) => {
        callback(e.detail, e, context);
    });
}
...
export default {};
export { toggle_class, document_load, dispatch, listen };